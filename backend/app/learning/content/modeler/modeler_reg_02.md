...
**第 1 题**某多元回归中 $X^TX = 4I_3$($p = 2$,含截距),$X^Ty = (16, 8, -4)^T$。最小二乘估计与拟合方程为:

A. $\hat y = 16 + 8x_1 - 4x_2$
B. $\hat y = 4 - x_1 + 2x_2$
C. $\hat y = 4 + 2x_1 - x_2$
D. $\hat y = 4 + 2x_1 + x_2$

<details><summary>查看答案与解析</summary>
**答案:C**。$(X^TX)^{-1} = \frac14 I$,故 $\hat\beta = \frac14(16, 8, -4) = (4, 2, -1)$,拟合方程 $\hat y = 4 + 2x_1 - x_2$。「$16 + 8x_1 - 4x_2$」漏乘了 $(X^TX)^{-1}$,直接把 $X^Ty$ 当系数;「$4 - x_1 + 2x_2$」把 $\beta$ 分量顺序颠倒;「$4 + 2x_1 + x_2$」把 $-4$ 的符号丢掉。列正交时每个系数退化成「一元公式」独立给出,这正是正交试验设计追求的效果。
</details>

**第 2 题**把变量 $x_3$ 对其余所有自变量做回归,得到 $R^2 = 0.9$。$x_3$ 的方差膨胀因子(VIF)为:
...
<details><summary>查看答案与解析</summary>
**答案:A**。$\mathrm{VIF} = \frac{1}{1 - R_j^2} = \frac{1}{1 - 0.9} = 10$,恰好到达「严重共线性」的常用警戒线(若 $R^2 = 0.95$,VIF = 20)。0.9 是直接抄 $R^2$;1.11 是 $\frac{1}{R^2}$ 方向的错误;9 是 $\frac{R^2}{1-R^2}$ 的记混。VIF 是软指标,要结合「整体 F 显著而单个 t 不显著、系数符号反常」等现象与业务理解综合判断。
</details>

**第 3 题**为什么不能用 $R^2$ 比较变量个数不同的模型?

A. 因为 $R^2$ 只在样本量足够大时才有效
B. 变量越多 $R^2$ 必然越大,必须用调整 $R^2$/AIC/BIC 等带惩罚的准则
C. 因为 $R^2$ 的上限是 1,无法区分好坏
D. 因为 $R^2$ 在多元回归中没有定义

<details><summary>查看答案与解析</summary>
**答案:B**。每加一个变量,SSE 只可能减小、$R^2$ 只可能增大——看 $R^2$ 永远会选全模型,直到过拟合。模型比较必须用带惩罚的准则:调整 $R^2$、AIC、BIC(数值小者优,BIC 对变量数惩罚更重)。「$R^2$ 在多元回归中没有定义」与事实相反,多元回归同样有 $R^2$,只是没有「方向」一说;「样本量足够大才有效」因果颠倒——样本量小的时候 $R^2$ 反而更容易虚高。
</details>
