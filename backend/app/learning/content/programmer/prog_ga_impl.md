...
**第 1 题(判断)**:为什么轮盘赌选择要求适应度非负?若目标是最小化问题,给出两种处理办法。

<details><summary>查看答案</summary>

轮盘赌按 $p_i = f_i / \sum f_j$ 抽样,负的 $f_i$ 会产生负概率(或经平移后概率失真),抽样语义崩溃。最小化问题两种办法:①**变换**:$f' = -f$(目标有下界时)或 $f' = 1/(1+f)$($f \geq 0$ 时);②**换选择算子**:锦标赛选择只比较大小,与正负无关,最省心。

</details>

**第 2 题(计算)**:10 位二进制编码、区间 $[0, 5]$,位串 `1010101010` 解码后的 $x$ 是多少?

<details><summary>查看答案</summary>

位串的十进制值 $D = 2^9 + 2^7 + 2^5 + 2^3 + 2^1 = 512 + 128 + 32 + 8 + 2 = 682$;

$$x = 0 + 5 \times \frac{682}{2^{10} - 1} = 5 \times \frac{682}{1023} \approx 3.333$$

注意分母是 $2^L - 1 = 1023$。位串全 1(`1111111111`)解码恰为 $5$、全 0 为 $0$,边界可达。

</details>

**第 3 题(补全)**:写出 TSP 排列编码的「交换变异」函数(输入一条路线,随机交换两个位置)。

<details><summary>查看答案</summary>

```python
def swap_mutate(route, rng):
    r = route.copy()
...
必须 `copy()`——直接改会污染原个体(注意 NumPy 切片/索引的视图语义,见《NumPy数组操作实战》单元);`replace=False` 保证两个位置不同,否则「变异了个寂寞」。

</details>

**第 4 题(概念)**:什么是早熟收敛?从收敛曲线上如何识别?给出两种对策。

<details><summary>查看答案</summary>

早熟收敛:种群多样性过早丧失,全部个体挤在某个局部最优附近,曲线早早躺平但结果远差于参考解(精确解/其他算法的解)。识别:收敛曲线在**前几十代就水平**,且多组不同随机种子都收敛到**同一个较差的值**——多样性枯竭的标志。对策:①提高变异率(二进制 $p_m$ 从 0.01 提到 0.05~0.1)或增大变异幅度(实数编码加大高斯 σ);②增大种群规模;③引入随机移民/重启机制;④改用更大 k 值的锦标赛或精英比例调低,减缓选择压力。
