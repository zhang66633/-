...
**第 1 题(判断)**:`np.vectorize(f)` 能让 `f` 的循环加速吗?什么场景用它?

<details><summary>查看答案</summary>

不能加速——`np.vectorize` 内部仍是 Python 层的逐元素调用,加上类型转换开销,往往比直接循环还慢。它的价值是**语义糖**:让标量函数接受数组输入、支持广播,方便代码书写,而不是性能。要性能:真向量化(把计算写进 NumPy 运算)或 Numba。

</details>

**第 2 题(计算)**:把下面的双重循环改写成广播向量化,并说明复杂度不变时为什么它更快:

```python
import numpy as np

n = 5
x = np.array([1.0, 3.0, 2.5, 4.0, 0.5])     # 题目设定
...
x = np.array([1.0, 3.0, 2.5, 4.0, 0.5])
D = np.abs(x[:, None] - x[None, :])
```

两个版本复杂度都是 $O(n^2)$(距离矩阵本身就有 $n^2$ 个元素),但向量化版本把 $n^2$ 次「Python 层循环 + 函数调用」替换成**一次**发生在 C 层的批量运算,常数因子缩小几十倍。另外注意:向量化版需要 $n^2$ 内存,若 $n$ 大到内存装不下,应改成分块计算或 Numba 循环——见例题 2。

</details>

**第 3 题(补全)**:给函数加 `@njit` 并说明第一次调用为什么慢:
...
第一次调用 `f(arr)` 时,Numba 把 Python 函数**编译**成机器码(0.1~1 秒),后续调用直接执行机器码;不同参数**类型**(如 float64 数组 vs int64 数组)会触发不同的编译版本。所以基准测试要「先预热一次再计时」;生产代码加 `@njit(cache=True)` 把编译结果缓存到磁盘,二次运行免编译。小数据量下单次调用可能因编译开销比纯 Python 还慢。

</details>

**第 4 题(概念)**:为什么说「优化性能优先改算法,而不是抠常数」?举例说明。

<details><summary>查看答案</summary>

复杂度差异是**数量级**的:排序问题 $O(n^2)$ 冒泡 → $O(n\log n)$ 快排在 $n=10^5$ 时差距约 $10^9$ vs $1.7\times10^6$ 次操作,任何常数级优化都无法弥补。常数级技巧(局部变量、缓存属性、in-place)最多 1~3 倍,且往往降低可读性。实战顺序:①确认瓶颈(测量);②降复杂度(数据结构/算法);③向量化或 JIT 编译;④最后才做常数级微调。例:求两两距离时,若只需找最近邻,用 kd-tree($O(n\log n)$)远比算完整距离矩阵($O(n^2)$)聪明。
