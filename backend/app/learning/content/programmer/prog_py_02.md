...
**第 1 题(判断)**:执行 `a = np.arange(10); b = a[3:7]; b[0] = 99` 后,`a` 是多少?

<details><summary>查看答案</summary>

`a` 变成 `[ 0  1  2 99  4  5  6  7  8  9]`。`b` 是 `a` 的视图,`b[0]` 即 `a[3]`。若想避免,用 `b = a[3:7].copy()`。这一题是「切片视图陷阱」的最小复现,务必亲手跑一遍。

</details>

**第 2 题(计算)**:不运行代码,写出 `np.ones((2, 3)) * np.array([1, 2, 3])` 的结果。

<details><summary>查看答案</summary>

$$ \begin{bmatrix} 1 & 2 & 3 \\ 1 & 2 & 3 \end{bmatrix} $$

`(2,3)` 与 `(3,)`(视作 (1,3)):第一维 2 对 1 → 复制,第二维 3 对 3 → 匹配。结果是 2×3 矩阵,每行都是 [1,2,3]。

</details>

**第 3 题(补全)**:用一行代码把矩阵 `X` 中所有负数替换为 0。
...
布尔掩码直接赋值。更通用的是 `np.where(X < 0, 0, X)`(返回新数组,不改原数组),两者语义不同,按需选择。

</details>

**第 4 题(计算)**:用 `np.linalg` 解方程组 $2x + y = 5,\ x + 3y = 10$,并验算残差。
...
解为 $x=1,\ y=3$(手动验证:$A$ 的行列式为 5)。数值解线性方程组后打印残差 `A @ x - b` 是必做动作——残差若不在 $10^{-12}$ 量级,说明矩阵病态,要考虑换 `np.linalg.lstsq` 或做正则化。

</details>

**第 5 题(实操)**:生成一个 5×4 的随机矩阵,实现「按行归一化」:每个元素除以该行元素之和。
...
X = rng.random((5, 4))
row_sum = X.sum(axis=1, keepdims=True)   # keepdims 保留 (5,1) 形状
Y = X / row_sum                          # 广播:(5,4) / (5,1)
print(Y.sum(axis=1))                     # 每行和为 1
```

关键在 `keepdims=True`:`sum(axis=1)` 默认返回 (5,),`X / row_sum` 会按列对齐广播而得到错误结果;保留 (5,1) 形状后广播方向才正确。这是「按行/按列归一化」的标准写法,也是马尔可夫链转移矩阵、权重归一化等场景的高频操作。
