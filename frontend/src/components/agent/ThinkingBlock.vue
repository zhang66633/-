...
<template>
  <Collapsible v-model:open="isOpen" :default-open="defaultOpen">
    <div class="flex items-center gap-1.5">
      <CollapsibleTrigger
        class="group flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors py-1 select-none"
      >
        <Brain class="h-3 w-3" />
    ...
    <CollapsibleContent>
      ...
      >
        <div class="prose prose-xs dark:prose-invert max-w-none break-words" v-html="renderedThinking" />
        <!-- 流式光标 -->
        <span
          v-if="streaming"
